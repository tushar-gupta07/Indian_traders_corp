...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$category_id) {
        $msg = 'Subcategory name and category are required.'; $msgType = 'error';
    } else {
        if (!$slug) $slug = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', $name));
        try {
            $stmt = $pdo->prepare("INSERT INTO menu_subcategories (category_id, name, slug, sort_order, is_active) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$category_id, $name, $slug, $sort, $active]);
            @unlink('../assets/cache/menu_tree.json');
            $msg = "Subcategory '<strong>$name</strong>' added successfully!"; $msgType = 'success';
        } catch (Exception $e) {
...
if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$id || !$category_id) {
        ...
        if (!$slug) $slug = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', $name));
        try {
            $stmt = $pdo->prepare("UPDATE menu_subcategories SET category_id=?, name=?, slug=?, sort_order=?, is_active=? WHERE id=?");
            $stmt->execute([$category_id, $name, $slug, $sort, $active, $id]);
            @unlink('../assets/cache/menu_tree.json');
            $msg = "Subcategory updated successfully!"; $msgType = 'success';
        } catch (Exception $e) {
            $msg = 'Error: ' . $e->getMessage(); $msgType = 'error';
        }
    }
...
if ($action === 'delete' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    try {
        $check = $pdo->prepare("SELECT COUNT(*) FROM menu_products WHERE subcategory_id=?");
        $check->execute([$id]);
        if ($check->fetchColumn() > 0) {
            $msg = 'Cannot delete! This subcategory has products linked to it. Delete or reassign products first.'; $msgType = 'error';
        } else {
            $pdo->prepare("DELETE FROM menu_subcategories WHERE id=?")->execute([$id]);
            @unlink('../assets/cache/menu_tree.json');
            $msg = 'Subcategory deleted successfully!'; $msgType = 'success';
        }
    } catch (Exception $e) {
        $msg = 'Error: ' . $e->getMessage(); $msgType = 'error';
    }
...
if ($action === 'toggle' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $pdo->prepare("UPDATE menu_subcategories SET is_active = 1 - is_active WHERE id=?")->execute([$id]);
    @unlink('../assets/cache/menu_tree.json');
    $redirect = isset($_GET['cat']) ? 'subcategories.php?cat='.(int)$_GET['cat'] : 'subcategories.php';
    header('Location: '.$redirect.'&msg=toggled'); exit;
}

// Filter by category (from categories.php "Subs" link)
...
$params = [];
$where  = [];
if ($filterCat) { $where[] = 's.category_id=?'; $params[] = $filterCat; }
if ($search)    { $where[] = 's.name LIKE ?';    $params[] = "%$search%"; }
$whereSQL = $where ? 'WHERE '.implode(' AND ', $where) : '';

$subs = $pdo->prepare("
    SELECT s.*, c.name AS cat_name,
           (SELECT COUNT(*) FROM menu_products WHERE subcategory_id=s.id) AS prod_count
...
$activeSubs = count(array_filter($subcategories, fn($s) => $s['is_active']));
...
<head>
    ...
    <title>Subcategories - ITC Admin</title>
...
<div class="sidebar flex flex-col" id="sidebar">
    ...
    <nav style="flex:1;padding:12px 10px;overflow-y:auto;">
        ...
        <a href="subcategories.php" class="sidebar-link active">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
                <h1 style="font-size:16px;font-weight:800;color:#0a2463;">
                    Subcategories<?= $currentCatName ? ' — ' . htmlspecialchars($currentCatName) : '' ?>
                </h1>
                <p style="font-size:11px;color:#94a3b8;font-weight:500;">Manage product subcategories under each category</p>
            </div>
        </div>
        <div style="display:flex;align-items:center;gap:12px;">
    ...
    <div style="padding:24px;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
            <div style="display:flex;gap:12px;flex-wrap:wrap;">
                <div style="background:white;border-radius:12px;padding:14px 20px;border:1px solid #e2e8f0;min-width:120px;">
                    <div style="font-size:22px;font-weight:800;color:#0a2463;"><?= $totalSubs ?></div>
                    <div style="font-size:11px;color:#94a3b8;font-weight:600;"><?= $filterCat ? 'In this Category' : 'Total Subcategories' ?></div>
                </div>
                <div style="background:white;border-radius:12px;padding:14px 20px;border:1px solid #e2e8f0;min-width:120px;">
                    <div style="font-size:22px;font-weight:800;color:#16a34a;"><?= $activeSubs ?></div>
                    <div style="font-size:11px;color:#94a3b8;font-weight:600;">Active</div>
                </div>
                <div style="background:white;border-radius:12px;padding:14px 20px;border:1px solid #e2e8f0;min-width:120px;">
                    <div style="font-size:22px;font-weight:800;color:#dc2626;"><?= $totalSubs - $activeSubs ?></div>
        ...
        <div style="background:white;border-radius:12px;border:1px solid #e2e8f0;padding:16px;margin-bottom:16px;">
            <form method="GET" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                ...
                <select name="cat" style="padding:8px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;background:white;color:#334155;font-weight:600;">
                ...
                <a href="subcategories.php" style="padding:8px 14px;border-radius:8px;border:1.5px solid #e2e8f0;font-size:13px;color:#64748b;text-decoration:none;font-weight:600;">Clear</a>
                <?php endif; ?>
            </form>
        </div>

        <!-- Breadcrumb if filtered -->
        <?php if ($filterCat && $currentCatName): ?>
        <div style="margin-bottom:14px;display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;color:#64748b;">
            ...
            <span>›</span>
            <span style="color:#334155;"><?= htmlspecialchars($currentCatName) ?></span>
            <span style="color:#94a3b8;font-size:11px;">(<?= $totalSubs ?> subcategories)</span>
        </div>
        <?php endif; ?>
                ...
                <table style="width:100%;border-collapse:collapse;">
                    <thead>
                        <tr style="background:#f8fafc;border-bottom:2px solid #e2e8f0;">
                            ...
                            <th style="padding:12px 16px;text-align:left;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Subcategory</th>
                            ...
                            <th style="padding:12px 16px;text-align:center;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Products</th>
                            <th style="padding:12px 16px;text-align:center;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.05em;">Sort</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="8" style="padding:48px;text-align:center;color:#94a3b8;font-size:14px;">
                                ...
                                <button onclick="openAddModal()" style="display:block;margin:12px auto 0;background:#0a2463;color:white;padding:8px 18px;border-radius:8px;border:none;cursor:pointer;font-size:13px;font-weight:600;">Add First Subcategory</button>
                            </td>
                        </tr>
                        <?php else: ?>
                        ...
                        <tr style="border-bottom:1px solid #f1f5f9;transition:background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                            ...
                            <td style="padding:14px 16px;">
                                <div style="font-size:14px;font-weight:700;color:#1e293b;"><?= htmlspecialchars($sub['name']) ?></div>
                            </td>
                            <td style="padding:14px 16px;">
                                <a href="subcategories.php?cat=<?= $sub['category_id'] ?>"
                                   style="display:inline-block;background:#e0e7ff;color:#0a2463;font-size:12px;font-weight:700;padding:3px 10px;border-radius:20px;text-decoration:none;">
                                    <?= htmlspecialchars($sub['cat_name']) ?>
                                </a>
                            </td>
                            <td style="padding:14px 16px;">
                                <code style="font-size:12px;background:#f1f5f9;padding:3px 8px;border-radius:6px;color:#475569;"><?= htmlspecialchars($sub['slug']) ?></code>
                            </td>
                            <td style="padding:14px 16px;text-align:center;">
                                <a href="products.php?sub=<?= $sub['id'] ?>"
                                   style="display:inline-block;background:#fef3c7;color:#92400e;font-size:12px;font-weight:700;padding:4px 12px;border-radius:20px;text-decoration:none;">
                                    <?= $sub['prod_count'] ?> products
                                </a>
                            </td>
                            <td style="padding:14px 16px;text-align:center;font-size:13px;font-weight:700;color:#64748b;"><?= $sub['sort_order'] ?></td>
                            <td style="padding:14px 16px;text-align:center;">
                                <?php $toggleUrl = 'subcategories.php?action=toggle&id='.$sub['id'].($filterCat?'&cat='.$filterCat:''); ?>
                                <a href="<?= $toggleUrl ?>" onclick="return confirm('Toggle status?')"
                                   class="<?= $sub['is_active'] ? 'badge-active' : 'badge-inactive' ?>">
                                    <?= $sub['is_active'] ? 'Active' : 'Inactive' ?>
                                </a>
                            </td>
                            <td style="padding:14px 16px;text-align:center;">
                                <div style="display:flex;gap:8px;justify-content:center;">
                                    <button onclick="openEditModal(<?= $sub['id'] ?>, '<?= addslashes($sub['name']) ?>', '<?= addslashes($sub['slug']) ?>', <?= $sub['category_id'] ?>, <?= $sub['sort_order'] ?>, <?= $sub['is_active'] ?>)"
                                    ...
                                    <a href="subcategories.php?action=delete&id=<?= $sub['id'] ?><?= $filterCat ? '&cat='.$filterCat : '' ?>"
                                       onclick="return confirm('Delete \'<?= addslashes($sub['name']) ?>\'? This cannot be undone!')"
...
<div class="modal-overlay" id="addModal">
    <div class="modal-box" style="background:white;border-radius:20px;width:100%;max-width:500px;padding:28px;margin:16px;box-shadow:0 32px 80px rgba(0,0,0,0.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
            <h2 style="font-size:18px;font-weight:800;color:#0a2463;">Add New Subcategory</h2>
        ...
        <form method="POST">
            ...
            <div style="margin-bottom:16px;">
                <label style="display:block;font-size:12px;font-weight:700;color:#475569;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em;">Parent Category *</label>
                <select name="category_id" id="addCategoryId" required
                        style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;background:white;color:#334155;box-sizing:border-box;">
            ...
            <div style="margin-bottom:16px;">
                <label style="display:block;font-size:12px;font-weight:700;color:#475569;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em;">Subcategory Name *</label>
                <input type="text" name="name" id="addName" required
                       oninput="autoSlug(this.value,'addSlug')"
                       placeholder="e.g. Ball Valves - Stainless Steel"
...
<div class="modal-overlay" id="editModal">
    <div class="modal-box" style="background:white;border-radius:20px;width:100%;max-width:500px;padding:28px;margin:16px;box-shadow:0 32px 80px rgba(0,0,0,0.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
            <h2 style="font-size:18px;font-weight:800;color:#0a2463;">Edit Subcategory</h2>
...
function openEditModal(id, name, slug, catId, sort, active) {
