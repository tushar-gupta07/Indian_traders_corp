...
if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $technical_specifications = trim($_POST['technical_specifications'] ?? '');
    ...
    if (!$name || !$subcategory_id) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (in_array($ext, ['jpg','jpeg','png','webp','gif'])) {
                ...
                $msg = 'Invalid image format.'; $msgType = 'error';
            }
        }
        ...
        if ($msgType !== 'error') {
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO menu_products (subcategory_id, subcategory2_id, name, slug, subtitle, description, image, displacement, pressure, applications, technical_specifications, sort_order, is_active) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
                $stmt->execute([$subcategory_id, $sub2val, $name, $slug, $subtitle, $description, $image, $displacement, $pressure, $applications, $technical_specifications, $sort, $active]);
                @unlink('../assets/cache/menu_tree.json');
                $msg = "Product '<strong>$name</strong>' added successfully!"; $msgType = 'success';
            } catch (Exception $e) {
...
if ($action === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$id || !$subcategory_id) {
        ...
        $existing = $pdo->prepare("SELECT image FROM menu_products WHERE id=?");
        $existing->execute([$id]);
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (in_array($ext, ['jpg','jpeg','png','webp','gif'])) {
                $filename = $slug . '-' . time() . '.' . $ext;
                if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadDir . $filename)) {
                    if ($image && file_exists('../assets/images/products/' . $image)) @unlink('../assets/images/products/' . $image);
                    $image = $filename;
                }
            }
        }
        ...
        try {
            $sub2val = $subcategory2_id > 0 ? $subcategory2_id : null;
            $stmt = $pdo->prepare("UPDATE menu_products SET subcategory_id=?, subcategory2_id=?, name=?, slug=?, subtitle=?, description=?, image=?, displacement=?, pressure=?, applications=?, technical_specifications=?, sort_order=?, is_active=? WHERE id=?");
            $stmt->execute([$subcategory_id, $sub2val, $name, $slug, $subtitle, $description, $image, $displacement, $pressure, $applications, $technical_specifications, $sort, $active, $id]);
            @unlink('../assets/cache/menu_tree.json');
...
if ($action === 'delete' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    try {
        $r = $pdo->prepare("SELECT image FROM menu_products WHERE id=?");
        $r->execute([$id]); $img = $r->fetchColumn();
        $pdo->prepare("DELETE FROM menu_products WHERE id=?")->execute([$id]);
        $pdo->prepare("DELETE FROM menu_product_specs WHERE product_id=?")->execute([$id]);
        if ($img && file_exists('../assets/images/products/' . $img)) @unlink('../assets/images/products/' . $img);
        @unlink('../assets/cache/menu_tree.json');
...
if ($action === 'toggle' && isset($_GET['id'])) {
    $pdo->prepare("UPDATE menu_products SET is_active = 1 - is_active WHERE id=?")->execute([(int)$_GET['id']]);
    @unlink('../assets/cache/menu_tree.json');
    $qs = http_build_query(array_filter(['sub'=>$filterSub,'sub2'=>$filterSub2,'cat'=>$filterCat]));
    header('Location: product_add.php'.($qs?'?'.$qs:'')); exit;
}
...
$where = []; $params = [];
if ($filterSub2) {
    $where[] = 'p.subcategory2_id=?'; $params[] = $filterSub2;
} elseif ($filterSub) {
    $where[] = 'p.subcategory_id=?'; $params[] = $filterSub;
} elseif ($filterCat) {
    $where[] = 'c.id=?'; $params[] = $filterCat;
}
...
$currentSubName = ''; $currentSub2Name = ''; $currentCatName = '';
if ($filterSub2) {
    $r = $pdo->prepare("SELECT s2.name, s.name AS sub, c.name AS cat FROM menu_subcategories2 s2 JOIN menu_subcategories s ON s2.subcategory_id=s.id JOIN menu_categories c ON s.category_id=c.id WHERE s2.id=?");
    $r->execute([$filterSub2]); $row = $r->fetch();
    if ($row) { $currentSub2Name = $row['name']; $currentSubName = $row['sub']; $currentCatName = $row['cat']; }
} elseif ($filterSub) {
...
<head>
    ...
    <style>
        ...
        .form-input { width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;box-sizing:border-box;transition:border-color 0.2s; }
        .form-input:focus { border-color:#0a2463; }
        .form-textarea { width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;box-sizing:border-box;resize:vertical;min-height:90px; }
        .form-textarea:focus { border-color:#0a2463; }
        .img-preview { width:80px;height:80px;object-fit:contain;border-radius:8px;border:1px solid #e2e8f0;background:#f8faff; }
        .sub2-badge { background:#f0fdf4;color:#166534;font-size:10px;font-weight:700;padding:2px 7px;border-radius:20px;border:1px solid #bbf7d0; }
        /* Hint boxes for textareas */
        .field-hint { font-size:11px;color:#94a3b8;margin-top:4px;line-height:1.4; }
        .field-hint code { background:#f1f5f9;padding:1px 5px;border-radius:4px;font-family:monospace;color:#475569; }
...
<div class="sidebar flex flex-col" id="sidebar">
    ...
    <nav style="flex:1;padding:12px 10px;overflow-y:auto;">
        ...
        <a href="product_add.php" class="sidebar-link active">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
                <h1 style="font-size:16px;font-weight:800;color:#0a2463;">
                    Products<?= $currentSub2Name ? ' — '.htmlspecialchars($currentSub2Name) : ($currentSubName ? ' — '.htmlspecialchars($currentSubName) : ($currentCatName ? ' — '.htmlspecialchars($currentCatName) : '')) ?>
                </h1>
                <p style="font-size:11px;color:#94a3b8;font-weight:500;">Manage all menu products</p>
            </div>
        </div>
        <div style="display:flex;align-items:center;gap:12px;">
    ...
    <div style="padding:24px;">
        ...
        <div style="background:white;border-radius:12px;border:1px solid #e2e8f0;padding:16px;margin-bottom:16px;">
            <form method="GET" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                ...
                <select name="cat" id="filterCat" onchange="filterSubs()" style="padding:8px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;background:white;color:#334155;font-weight:600;">
                    <option value="">All Categories</option>
                    <?php foreach ($allCats as $c): ?>
                    <option value="<?= $c['id'] ?>" <?= $filterCat==$c['id']?'selected':'' ?>><?= htmlspecialchars($c['name']) ?></option>
                    <?php endforeach; ?>
                </select>
                <select name="sub" id="filterSub" onchange="filterSub2s()" style="padding:8px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;background:white;color:#334155;font-weight:600;">
                ...
                <select name="sub2" id="filterSub2" style="padding:8px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;background:white;color:#334155;font-weight:600;">
                ...
                <a href="product_add.php" style="padding:8px 14px;border-radius:8px;border:1.5px solid #e2e8f0;font-size:13px;color:#64748b;text-decoration:none;font-weight:600;">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div style="margin-bottom:14px;display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;color:#64748b;flex-wrap:wrap;">
            <a href="categories.php" style="color:#0a2463;text-decoration:none;">Categories</a>
            <?php if ($currentCatName): ?><span>›</span><a href="product_add.php?cat=<?= $filterCat ?>" style="color:#0a2463;text-decoration:none;"><?= htmlspecialchars($currentCatName) ?></a><?php endif; ?>
            <?php if ($currentSubName): ?><span>›</span><a href="product_add.php?sub=<?= $filterSub ?>" style="color:#0a2463;text-decoration:none;"><?= htmlspecialchars($currentSubName) ?></a><?php endif; ?>
            <?php if ($currentSub2Name): ?><span>›</span><span style="color:#334155;"><?= htmlspecialchars($currentSub2Name) ?></span><?php endif; ?>
            <span style="color:#94a3b8;font-size:11px;">(<?= $totalProds ?> products)</span>
        </div>
        <?php endif; ?>
            ...
            <div style="overflow-x:auto;">
                <table style="width:100%;border-collapse:collapse;">
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="8" style="padding:48px;text-align:center;color:#94a3b8;font-size:14px;">
                            No products found.
                        ...
                        <?php
                            $imgSrc = !empty($prod['image']) ? '../assets/images/products/'.$prod['image'] : '../assets/images/Gate-Valve-1.png';
                            $qs = http_build_query(array_filter(['sub'=>$filterSub,'sub2'=>$filterSub2,'cat'=>$filterCat]));
                            $toggleUrl = 'product_add.php?action=toggle&id='.$prod['id'].($qs?'&'.$qs:'');
                            $deleteUrl = 'product_add.php?action=delete&id='.$prod['id'].($qs?'&'.$qs:'');
                        ?>
                        <tr style="border-bottom:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                            <td style="padding:12px 16px;font-size:13px;font-weight:700;color:#94a3b8;"><?= $i+1 ?></td>
                            <td style="padding:12px 16px;">
                                <img src="<?= htmlspecialchars($imgSrc) ?>"
                                     style="width:48px;height:48px;object-fit:contain;border-radius:8px;border:1px solid #e2e8f0;background:#f8faff;"
                            ...
                            <td style="padding:12px 16px;max-width:220px;">
                                ...
                                <div style="font-size:11px;color:#94a3b8;margin-top:2px;"><?= htmlspecialchars(substr($prod['subtitle'],0,50)) ?><?= strlen($prod['subtitle'])>50?'...':'' ?></div>
                                <?php endif; ?>
                                <code style="font-size:10px;color:#64748b;"><?= htmlspecialchars($prod['slug']) ?></code>
                                <!-- Show badges if applications / tech specs filled -->
                                <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px;">
                                    <?php if (!empty($prod['applications'])): ?>
                                    <span style="background:#f0fdf4;color:#166534;font-size:10px;font-weight:700;padding:1px 6px;border-radius:10px;border:1px solid #bbf7d0;">Apps ✓</span>
                                    <?php endif; ?>
                                    <?php if (!empty($prod['technical_specifications'])): ?>
                                    <span style="background:#eff6ff;color:#1d4ed8;font-size:10px;font-weight:700;padding:1px 6px;border-radius:10px;border:1px solid #bfdbfe;">Tech Specs ✓</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td style="padding:12px 16px;">
                                <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
                                    ...
                                    <span style="color:#94a3b8;">›</span>
                                    <span class="sub2-badge"><?= htmlspecialchars($prod['sub2_name']) ?></span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td style="padding:12px 16px;text-align:center;">
                                <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                                    ...
                                    <a href="../products_details.php?slug=<?= urlencode($prod['slug']) ?>" target="_blank"
                                       style="background:#f0fdf4;color:#166534;padding:6px 10px;border-radius:8px;font-size:12px;font-weight:700;text-decoration:none;">👁️ View</a>
                                    <a href="specifications.php?product_id=<?= $prod['id'] ?>"
                                    ...
                                    <a href="<?= $deleteUrl ?>" onclick="return confirm('Delete \'<?= addslashes($prod['name']) ?>\'?')"
...
<div class="modal-overlay" id="addModal">
    <div class="modal-box" style="background:white;border-radius:20px;width:100%;max-width:680px;padding:28px;box-shadow:0 32px 80px rgba(0,0,0,0.2);">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                <div>
                    ...
                    <select id="addCatF" onchange="filterModalSubs('add')"
                ...
                <div>
                    ...
                    <select name="subcategory_id" id="addSubF" required onchange="filterModalSub2s('add')"
                            style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;background:white;box-sizing:border-box;">
            ...
            <div style="margin-bottom:14px;">
                <label class="form-label">Subcategory2 <span style="font-weight:400;color:#94a3b8;text-transform:none;font-size:11px;">(Optional)</span></label>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                <div>
                    ...
                    <input type="text" name="displacement" placeholder="e.g. 50-400 cc/rev" class="form-input">
                </div>
                <div>
                    <label class="form-label">Pressure</label>
                    <input type="text" name="pressure" placeholder="e.g. 250 bar max" class="form-input">
                </div>
            </div>
            ...
            <div style="margin-bottom:14px;">
                <label class="form-label" style="color:#166534;">
                    🏭 Applications
                    <span style="font-weight:400;color:#94a3b8;text-transform:none;font-size:11px;">(Optional)</span>
                </label>
                <textarea name="applications" class="form-textarea" style="min-height:100px;border-color:#bbf7d0;" placeholder="One application per line:&#10;Oil & Gas Industry&#10;Power Generation Plants&#10;Marine & Offshore Applications&#10;Chemical Processing"></textarea>
                <p class="field-hint">Ek line mein ek application likho. Ye product page pe <code>Applications</code> tab mein dikhega.</p>
            </div>

            <!-- ══ NEW: Technical Specifications ══ -->
            <div style="margin-bottom:14px;">
                <label class="form-label" style="color:#1d4ed8;">
                    🔧 Technical Specifications
                    <span style="font-weight:400;color:#94a3b8;text-transform:none;font-size:11px;">(Optional)</span>
                </label>
                <textarea name="technical_specifications" class="form-textarea" style="min-height:120px;border-color:#bfdbfe;font-family:monospace;font-size:13px;" placeholder="Key: Value format (one per line):&#10;Body Material: Cast Iron / Ductile Iron&#10;End Connection: Flanged (IS 1538)&#10;Pressure Rating: PN10 / PN16&#10;Temperature Range: -10°C to 120°C&#10;Standards: IS 780 / BS 5150"></textarea>
                <p class="field-hint"><code>Key: Value</code> format mein likho, ek line mein ek spec. Ye <code>Technical Specs</code> tab mein table ke roop mein dikhega.</p>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                ...
                <div>
                    ...
                    <input type="file" name="image" accept="image/*" onchange="previewImg(this,'addImgPrev')"
                           style="width:100%;padding:8px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:13px;box-sizing:border-box;">
                    <img id="addImgPrev" src="" style="display:none;margin-top:8px;" class="img-preview">
                </div>
            </div>

            <div style="margin-bottom:20px;display:flex;align-items:center;gap:10px;">
                <input type="checkbox" name="is_active" id="addActive" checked style="width:16px;height:16px;cursor:pointer;">
                <label for="addActive" style="font-size:13px;font-weight:600;color:#475569;cursor:pointer;">Active (visible on website)</label>
            </div>
    ...
    <div class="modal-box" style="background:white;border-radius:20px;width:100%;max-width:680px;padding:28px;box-shadow:0 32px 80px rgba(0,0,0,0.2);">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                <div>
                    <label class="form-label">Category</label>
                    <select id="editCatF" onchange="filterModalSubs('edit')"
                            style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;background:white;box-sizing:border-box;">
                        <option value="">All Categories</option>
                        <?php foreach ($allCats as $c): ?>
                        <option value="<?= $c['id'] ?>"><?= htmlspecialchars($c['name']) ?></option>
                ...
                <div>
                    <label class="form-label">Subcategory *</label>
                    <select name="subcategory_id" id="editSubF" required onchange="filterModalSub2s('edit')"
                            style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;outline:none;background:white;box-sizing:border-box;">
                        <?php foreach ($allSubs as $s): ?>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                <div>
                    <label class="form-label">Displacement</label>
                    <input type="text" name="displacement" id="editDisplacement" class="form-input">
                </div>
                <div>
                    <label class="form-label">Pressure</label>
                    <input type="text" name="pressure" id="editPressure" class="form-input">
                </div>
            </div>
            ...
            <div style="margin-bottom:14px;">
                ...
                <textarea name="applications" id="editApplications" class="form-textarea" style="min-height:100px;border-color:#bbf7d0;" placeholder="One application per line:&#10;Oil & Gas Industry&#10;Power Generation Plants"></textarea>
                <p class="field-hint">Ek line mein ek application likho.</p>
            </div>
            ...
            <div style="margin-bottom:14px;">
                ...
                <textarea name="technical_specifications" id="editTechSpecs" class="form-textarea" style="min-height:120px;border-color:#bfdbfe;font-family:monospace;font-size:13px;" placeholder="Key: Value format:&#10;Body Material: Cast Iron&#10;Pressure Rating: PN16"></textarea>
                <p class="field-hint"><code>Key: Value</code> format mein likho, ek line mein ek spec.</p>
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
                <div>
                    <label class="form-label">Sort Order</label>
                    <input type="number" name="sort_order" id="editSort" min="0" class="form-input">
                </div>
                <div>
                    <label class="form-label">New Image (optional)</label>
                    <input type="file" name="image" accept="image/*" onchange="previewImg(this,'editImgPrev')"
                           style="width:100%;padding:8px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:13px;box-sizing:border-box;">
                    <img id="editImgPrev" src="" style="margin-top:8px;" class="img-preview">
                </div>
            </div>

            <div style="margin-bottom:20px;display:flex;align-items:center;gap:10px;">
                <input type="checkbox" name="is_active" id="editActive" style="width:16px;height:16px;cursor:pointer;">
                <label for="editActive" style="font-size:13px;font-weight:600;color:#475569;cursor:pointer;">Active (visible on website)</label>
            </div>
            <div style="display:flex;gap:10px;">
...
function filterModalSubs(prefix) {
    ...
    filterModalSub2s(prefix);
}

function filterModalSub2s(prefix) {
...
function filterSub2s() {
    var subId = document.getElementById('filterSub').value;
    var sel   = document.getElementById('filterSub2');
    var cur   = sel.value;
...
function openEditModal(prod) {
    ...
    document.getElementById('editSlug').value            = prod.slug;
    document.getElementById('editSubtitle').value        = prod.subtitle         || '';
    document.getElementById('editDescription').value     = prod.description      || '';
    document.getElementById('editDisplacement').value    = prod.displacement     || '';
    document.getElementById('editPressure').value        = prod.pressure         || '';
    document.getElementById('editApplications').value   = prod.applications     || '';
    document.getElementById('editTechSpecs').value      = prod.technical_specifications || '';
    document.getElementById('editSort').value            = prod.sort_order;
    document.getElementById('editActive').checked        = prod.is_active == 1;

    var imgEl = document.getElementById('editImgPrev');
    imgEl.src = prod.image ? '../assets/images/products/'+prod.image : '../assets/images/Gate-Valve-1.png';
    imgEl.style.display = 'block';

    var subData = allSubsData.find(function(s){ return s.id == prod.subcategory_id; });
    if (subData) {
        document.getElementById('editCatF').value = subData.cat_id;
        filterModalSubs('edit');
    }
    document.getElementById('editSubF').value = prod.subcategory_id;
    filterModalSub2s('edit');
    if (prod.subcategory2_id) {
        document.getElementById('editSub2F').value = prod.subcategory2_id;
    }
