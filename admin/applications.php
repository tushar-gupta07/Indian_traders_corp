...
$pageSubtitle = 'Manage product applications';
$activePage   = 'applications';

$products = [];
$res = $conn->query("SELECT p.id, p.name, p.category, p.image, COUNT(pa.id) as app_count FROM products p LEFT JOIN product_applications pa ON p.id=pa.product_id WHERE p.is_active=1 GROUP BY p.id ORDER BY p.sort_order");
...
<div style="background:white;border-radius:16px;border:1.5px solid #e2e8f0;overflow:hidden;">
    <div style="background:#f8fafc;padding:16px 20px;border-bottom:2px solid #e2e8f0;">
        <h3 style="font-size:14px;font-weight:800;color:#0a2463;">🏭 Product Applications</h3>
    </div>
    <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#f8fafc;border-bottom:1.5px solid #e2e8f0;">
            ...
            <th style="padding:12px 16px;text-align:center;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;">Applications</th>
        ...
        <tr style="border-bottom:1px solid #f1f5f9;" onmouseover="this.style.background='#f8faff'" onmouseout="this.style.background='white'">
            ...
            <td style="padding:12px 16px;text-align:center;font-size:14px;font-weight:800;color:<?php echo $p['app_count']>0?'#16a34a':'#b71c1c'; ?>;"><?php echo $p['app_count']; ?></td>
            <td style="padding:12px 16px;text-align:center;">
                <a href="product_edit.php?id=<?php echo $p['id']; ?>&tab=applications" style="background:#0a2463;color:white;padding:6px 14px;border-radius:7px;font-size:11px;font-weight:700;text-decoration:none;">Manage →</a>
            </td>
        </tr>
        <?php endforeach; ?>
        </tbody>
